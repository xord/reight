...
スプライトエディターとマップエディターを実装した時点では起動時はどちらかの画面を固定で表示し切り替えられない状態でした。しかしレトロゲーム開発の統合開発環境とするために各エディターを切り替えながら利用できるする必要があり。

こちらは統合環境可実装中に、実際にスプライトエディターとマップエディターを切り替えて使う様子です。<br>
